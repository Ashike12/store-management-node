...
export function buildMongoUri(config: ConfigService): string {
  ...
  const protocol =
    trimOptional(config.get<string>('DB_PROTOCOL')) || 'mongodb+srv';
  const hosts =
    trimOptional(config.get<string>('DB_HOSTS')) ||
    trimOptional(config.get<string>('DB_HOST'));
  ...
  if (!hosts) {
    throw new Error(
      'Mongo configuration missing. Set DB_HOSTS (or DB_HOST) and DB_NAME, or provide DB_URI.',
    );
  }
